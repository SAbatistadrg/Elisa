...
class OCRAnalyzer:
    ...
    def extract_slider_value(self, image_path):
        """Extrai o valor numérico do slider - PRIORIZA NÚMEROS COM PONTO"""
        try:
            ...
            all_texts = []
            numbers_with_dot = []  # PRIORIDADE: números com ponto
            numbers_without_dot = []  # BACKUP: números sem ponto
            ...
            for (bbox, text, confidence) in results:
                ...
                for num_str in numbers:
                    if '.' in num_str:  # TEM PONTO - PRIORIDADE!
                        try:
                            value = float(num_str)
                            numbers_with_dot.append((value, confidence))
                            print(f"         → Número COM PONTO encontrado: {value}")
                        except:
                            pass
                    else:  # SEM PONTO - BACKUP
                        try:
                            value = float(num_str)
                            numbers_without_dot.append((value, confidence))
                            print(f"         → Número sem ponto encontrado: {value}")
                        except:
                            pass

            full_text = " | ".join(all_texts) if all_texts else "N/A"

            # PRIORIZA números com ponto
            if numbers_with_dot:
                # Pega o de maior confiança
                best = max(numbers_with_dot, key=lambda x: x[1])
                print(f"   ✓ Valor extraído (COM PONTO): {best[0]}")
                return best[0], full_text

            # Se não achou com ponto, usa sem ponto
            if numbers_without_dot:
                best = max(numbers_without_dot, key=lambda x: x[1])
                print(f"   ✓ Valor extraído (sem ponto): {best[0]}")
                return best[0], full_text
